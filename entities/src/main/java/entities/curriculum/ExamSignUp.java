...
@NamedQueries(value = {
        ...
        @NamedQuery(name = "ExamSignUp.getNumberOfExamTakingsBeforeStudyYear", query = "SELECT COUNT(es) FROM exam_sign_up es WHERE es.studentCourses.enrolment.student.id = :student_id AND es.studentCourses.course.id = :course_id AND es.studentCourses.enrolment.studyYear.id < :studyYearId AND es.returned = false"),
