...
public class StudyFormSource {
    ...
    @GET
    public Response getStudyForms(@QueryParam("search") String searchQuery) {
        QueryParameters query = QueryParameters.query(uriInfo.getRequestUri().getQuery()).build();
        List enl = cB.getStudyForms(query, searchQuery);
        return Response
                .ok(enl)
                .header("X-Total-Count", cB.getStudyForms(new QueryParameters(), searchQuery).size())
                .build();
    }
