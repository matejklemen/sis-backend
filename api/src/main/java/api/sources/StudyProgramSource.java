...
public class StudyProgramSource {
    ...
    @GET
    public Response getStudyPrograms(@QueryParam("search") String searchQuery) {
        QueryParameters query = QueryParameters.query(uriInfo.getRequestUri().getQuery()).build();
        List enl = cb.getStudyPrograms(query, searchQuery);
        return Response
                .ok(enl)
                .header("X-Total-Count", cb.getStudyPrograms(new QueryParameters(), searchQuery).size())
                .build();
    }
