...
public class StudyYearSource {
    ...
    @GET
    public Response getStudyYears(@QueryParam("search") String searchQuery) {
        QueryParameters query = QueryParameters.query(uriInfo.getRequestUri().getQuery()).build();
        List enl = cb.getStudyYears(query, searchQuery);
        return Response
                .ok(enl)
                .header("X-Total-Count", cb.getStudyYears(new QueryParameters(), searchQuery).size())
                .build();
    }
