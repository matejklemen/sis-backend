...
public class MunicipalitySource {
    ...
    @GET
    public Response getMunicipalities(@QueryParam("search") String searchQuery) {
        QueryParameters query = QueryParameters.query(uriInfo.getRequestUri().getQuery()).build();
        List enl = pab.getMunicipalities(query, searchQuery);
        return Response
                .ok(enl)
                .header("X-Total-Count", pab.getMunicipalities(new QueryParameters(), searchQuery).size())
                .build();
    }
